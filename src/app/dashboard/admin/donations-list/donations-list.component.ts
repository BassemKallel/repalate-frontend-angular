...
export class DonationsListComponent implements OnInit {
  displayedColumns = ['id', 'donor', 'association', 'type', 'stock', 'createdAt', 'status'];
  ...
  computeStats(announcements: Announcement[]): StatCard[] {
    ...
    const completed = announcements.filter((a) => a.moderationStatus === 'ACCEPTED').length;
    const pending = announcements.filter((a) => a.moderationStatus === 'PENDING').length;
    const cancelled = announcements.filter((a) => a.moderationStatus === 'DELETED' || a.moderationStatus === 'REJECTED').length;
    return [
